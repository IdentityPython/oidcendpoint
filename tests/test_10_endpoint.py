...
def pre(endpoint_context, args, request):
    args.update({'name': '{}, {}'.format(args['family_name'],
                                         args['given_name'])})
    return args


def post(endpoint_context, cis, request):
    cis['request'] = request
    return cis
...
class TestEndpoint(object):
    ...
    def test_parse_urlencoded(self):
        ...
        req = self.endpoint.parse_request(self.endpoint_context, request)
        assert req == REQ
    ...
    def test_construct(self):
        msg = self.endpoint.construct(self.endpoint_context, EXAMPLE_MSG, {})
    ...
    def test_do_response_body_json(self):
        ...
        msg = self.endpoint.do_response(self.endpoint_context, EXAMPLE_MSG)
